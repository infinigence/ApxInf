"""Data-source adapters for π0-FAST calibration Observations.
...
def load_libero_observations(
    ...
    *,
    ...
    ``finger_joints`` is a checkpoint property, not a preference: π0-FAST's
    ``observation.state`` statistics are 8 wide, so its state keeps both of
    LIBERO's mirrored finger joints. Passing the wrong width is a wrong prompt
    and every captured scale would describe an input the policy never sees.
