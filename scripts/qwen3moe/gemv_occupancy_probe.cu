...
int main(){
 ...
 for(auto c:{Case{2048,5120,1,4,false,false},Case{4096,2048,1,8,false,false},Case{2048,1536,8,2,false,false},Case{768,2048,8,1,true,true},Case{2048,5120,1,3,false,false},Case{768,768,17,3,true,true}}){
  ...
  auto candidate=w4a16_gemv_schedule_kernel<true,false,false,4,4>;
  decltype(candidate) variants[]={candidate,candidate,w4a16_gemv_schedule_kernel<true,false,false,8,4>,w4a16_gemv_schedule_kernel<true,false,false,16,4>,w4a16_gemv_schedule_kernel<true,false,false,16,3>,w4a16_gemv_schedule_kernel<true,false,false,16,2>,w4a16_gemv_schedule_kernel<true,false,false,8,3>,w4a16_gemv_schedule_kernel<true,false,false,4,3>,w4a16_gemv_schedule_kernel<true,false,true,16,3>,w4a16_gemv_schedule_kernel<true,false,true,4,4>};
  auto launch=[&](int mode){
    ...
    else variants[mode]<<<mode<8?grid:dim3(56),256,shared>>>(dx,c.separate?c.k:0,reinterpret_cast<int32_t*>(packed),dz,ds,ids_ptr,sq,sz,ss,c.scaled?dw:nullptr,out,c.k,pc,128,rp,c.splits,c.slots);
  };
  ...
  cudaEvent_t start,end;CHECK(cudaEventCreate(&start));CHECK(cudaEventCreate(&end));float times[10]={};
  for(int mode=0;mode<10;++mode) {
    cudaFuncAttributes attr;int resident;CHECK(cudaFuncGetAttributes(&attr,mode?reinterpret_cast<const void*>(variants[mode]):reinterpret_cast<const void*>(w4a16_gemv_magic_kernel<true>)));CHECK(cudaOccupancyMaxActiveBlocksPerMultiprocessor(&resident,mode?reinterpret_cast<const void*>(variants[mode]):reinterpret_cast<const void*>(w4a16_gemv_magic_kernel<true>),256,shared));
    ...
    printf("K=%d N=%d slots=%d splits=%d mode=%d grid=%d operator_ms=%g bit_mismatches=%d registers=%d resident_ctas=%d\n",c.k,c.n,c.slots,c.splits,mode,mode<8?int(grid.x*grid.y*grid.z):56,times[mode],bad,attr.numRegs,resident);if(bad)return 2;
  }
