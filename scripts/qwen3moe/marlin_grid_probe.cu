...
int main() {
  for (int M : {1024,8192}) for (int layout : {0,1,2}) for (int K : {768,2048}) {
    ...
    auto fn=constant;
    int grids[]={28,14,42,56,84,112,224};
    int shared=65536,threads=128,grid=grids[mode];
    ...
    printf("M=%d layout=%d topk=%d K=%d N=%d max_error=%g bad=%d bit_mismatches=%d mapped_weights=1\n",M,layout,topk,K,N,maxerr,bad,byte_bad);if(bad)return 3;  // Split changes may change FP32 reduction; report bytes separately.
    }
