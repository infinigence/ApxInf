...
int main(int argc,char** argv){
  ...
  auto fn=mode?gqa_decode_vector_bf16_kernel<true>:gqa_decode_vector_bf16_kernel<false>;
  const int shared=0;
  const int capacity=9216,heads=32,kvheads=4,splits=argc>2?atoi(argv[2]):16;
  ...
  for(int length:{1,31,32,33,127,256,512,513,1024,1025,1152,2048,2049,4096,4097,8192,8193}){
    uint32_t position=length-1;CHECK(cudaMemcpy(pos,&position,4,cudaMemcpyHostToDevice));
    auto launch=[&](){fn<<<dim3(kvheads,splits),256,shared,stream>>>(dq,dk,dv,partial,pos,capacity,splits,kvheads,1.f/sqrtf(128));gqa_decode_combine_bf16_kernel<<<heads,128,0,stream>>>(partial,out,heads,splits);};
    ...
    if(mode==1) {
      gqa_decode_vector_bf16_kernel<<<dim3(kvheads,splits),256,0,stream>>>(dq,dk,dv,partial,pos,capacity,splits,kvheads,1.f/sqrtf(128));
      ...
      printf("bit_length=%d mismatches=%d\n",length,mismatches);if(length<=512 && mismatches)return 3;
    }
    ...
    printf("mode=%d splits=%d length=%d ms=%g max_error=%g bad=%d\n",mode,splits,length,ms,maxerror,bad);if(bad)return 2;CHECK(cudaEventDestroy(begin));CHECK(cudaEventDestroy(end));CHECK(cudaGraphExecDestroy(executable));CHECK(cudaGraphDestroy(graph));
  }
