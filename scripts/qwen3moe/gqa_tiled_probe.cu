...
int main(int argc,char** argv){
  ...
  if(tile!=0 && tile!=32 && tile!=64 && tile!=128)return 2;
  auto fn=tile==64?gqa_decode_tiled_bf16_kernel<64>:tile==128?gqa_decode_tiled_bf16_kernel<128>:gqa_decode_tiled_bf16_kernel<32>;
  const int shared=16*136*2+tile*136*2+tile*128*2+16*tile*4+2*16*(tile+8)*2+16*128*4;
  CHECK(cudaFuncSetAttribute(fn,cudaFuncAttributeMaxDynamicSharedMemorySize,shared));
  ...
  for(int length:{1,31,32,33,127,1024,4096,8192}){
    ...
    cudaEvent_t begin,end;CHECK(cudaEventCreate(&begin));CHECK(cudaEventCreate(&end));CHECK(cudaEventRecord(begin,stream));for(int i=0;i<100;++i)CHECK(cudaGraphLaunch(executable,stream));CHECK(cudaEventRecord(end,stream));CHECK(cudaEventSynchronize(end));float ms;CHECK(cudaEventElapsedTime(&ms,begin,end));
    ...
    printf("length=%d ms=%g max_error=%g bad=%d\n",length,ms/100,maxerror,bad);if(bad)return 2;CHECK(cudaEventDestroy(begin));CHECK(cudaEventDestroy(end));CHECK(cudaGraphExecDestroy(executable));CHECK(cudaGraphDestroy(graph));
  }
  CHECK(cudaStreamDestroy(stream));CHECK(cudaFree(dq));CHECK(cudaFree(dk));CHECK(cudaFree(dv));CHECK(cudaFree(out));CHECK(cudaFree(partial));CHECK(cudaFree(pos));
}
