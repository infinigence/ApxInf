// Batched expert partial reduction: CPU sum + original norm is the oracle.
// nvcc -O3 -arch=sm_101 scripts/qwen3moe/partial_rows_probe.cu -o partial_rows_probe
...
int main(){
 for(int rows:{1,5,28,128,256})for(int cols:{2048,4096})for(int count:{8,16}){
  uint32_t seed=52;auto random=[&](){seed=seed*1664525+1013904223;return float(int(seed>>16)-32768)/32768.f;};
  int elements=rows*cols;
  std::vector<float> parts(elements*count);std::vector<__nv_bfloat16>x(elements),weight(cols),delta(elements);
  for(auto&p:parts)p=random();for(auto&p:x)p=__float2bfloat16(random());for(auto&p:weight)p=__float2bfloat16(1+random());
  // Independent CPU FP32 reduction followed by the unchanged residual norm.
  for(int row=0;row<rows;++row)for(int i=0;i<cols;++i){float value=0;for(int c=0;c<count;++c)value+=parts[(int64_t(row)*count+c)*cols+i];delta[row*cols+i]=__float2bfloat16(value);}
  auto dp=upload(parts);auto dx=upload(x),rx=upload(x),dw=upload(weight),dd=upload(delta);__nv_bfloat16* out,*ref;CHECK(cudaMalloc(&out,elements*2));CHECK(cudaMalloc(&ref,elements*2));
  reference_residual_norm<<<rows,256,cols*4>>>(rx,dd,dw,ref,cols,rows,1e-6f);
  partial_residual_rms_bf16_kernel<<<rows,256,cols*4>>>(dx,dp,dw,out,cols,rows,count,1e-6f);CHECK(cudaDeviceSynchronize());
  std::vector<__nv_bfloat16> got(elements),expected(elements),gx(elements),ex(elements);
  CHECK(cudaMemcpy(got.data(),out,elements*2,cudaMemcpyDeviceToHost));CHECK(cudaMemcpy(expected.data(),ref,elements*2,cudaMemcpyDeviceToHost));CHECK(cudaMemcpy(gx.data(),dx,elements*2,cudaMemcpyDeviceToHost));CHECK(cudaMemcpy(ex.data(),rx,elements*2,cudaMemcpyDeviceToHost));
  int bad=0;for(int i=0;i<elements;++i){bad+=memcmp(&got[i],&expected[i],2)!=0;bad+=memcmp(&gx[i],&ex[i],2)!=0;}
  printf("rows=%d cols=%d count=%d bit_mismatches=%d\n",rows,cols,count,bad);if(bad)return 2;
