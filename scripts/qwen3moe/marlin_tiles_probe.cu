...
int main() {
  for (int K : {768,2048}) {
    ...
    auto fn=apxinf_marlin_probe::Marlin<half,vllm::kU4.id(),vllm::kFloat16.id(),128,2,16,4,false,4,8,false>;
    CUDA(cudaFuncSetAttribute(fn,cudaFuncAttributeMaxDynamicSharedMemorySize,53248));
    auto launch=[&](){fn<<<42,128,53248>>>(reinterpret_cast<int4*>(da),reinterpret_cast<int4*>(packed),reinterpret_cast<int4*>(out),tmp,nullptr,reinterpret_cast<int4*>(dsp),nullptr,reinterpret_cast<int4*>(dzp),nullptr,dsorted,de,dc,nullptr,1,false,false,K/128,M,N,K,locks,false,false,true,53248);};
    launch();
    CUDA(cudaGetLastError());CUDA(cudaDeviceSynchronize());
