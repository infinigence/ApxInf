...
int main(){
 ...
 for(int rows:{1,17,128,1024}){
  ...
  routed_residual_rms_bf16_kernel<<<rows,256,cols*4>>>(dx,dy,di,dr,dw,out,cols,rows,topk,1e-6f);CHECK(cudaDeviceSynchronize());
  ...
  for(void*p:{(void*)dx,(void*)rx,(void*)dy,(void*)dw,(void*)di,(void*)dr,(void*)out,(void*)delta,(void*)ref})CHECK(cudaFree(p));
 }
 return 0;
}
