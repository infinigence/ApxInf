...
int main(int argc,char** argv){
  ...
  for(int length:{1,31,32,33,127,1024,4096,8192}){
    ...
    auto launch=[&](){gqa_decode_partial_bf16_kernel<<<dim3(kvheads,splits),256,0,stream>>>(dq,dk,dv,partial,pos,capacity,splits,kvheads,1.f/sqrtf(128));gqa_decode_combine_bf16_kernel<<<heads,128,0,stream>>>(partial,out,heads,splits);};
