...
// Diagnostic derivative of w4a16_pair.cuh, SHA256 a2221b4181371442f43505331143310a754c5c47a4ff2b6bf3dae87c59f1e056
// Exact paired INT4 conversion reuses the vendored Marlin dequant helper.
template<int fixed_rows=0, int fixed_cols=0, int fixed_block=0, int min_ctas=3>
__global__ void __launch_bounds__(256,min_ctas) w4a16_gemv_constants_kernel(
    ...
    int rows_arg, int packed_cols_arg, int group_size_arg, int rows_per_block_arg) {
  constexpr bool blocked=true, pdl=false, vector_red=true;
  const int rows=fixed_rows?fixed_rows:rows_arg;
  const int packed_cols=fixed_cols?fixed_cols:packed_cols_arg;
  const int rows_per_block=fixed_block?fixed_block:rows_per_block_arg;
  ...
  const bool active = true;  // caller requires complete 32-word column tiles
