...
int main(){
 ...
 for(auto c:{Case{2048,5120,1,4,false,false},Case{4096,2048,1,8,false,false},Case{2048,1536,8,2,false,false},Case{768,2048,8,1,true,true},Case{2048,5120,1,3,false,false}}){
  ...
  auto original=[&](){w4a16_gemv_partial_kernel<<<grid,256,shared>>>(dx,c.separate?c.k:0,dq,dz,ds,di,sq,sz,ss,c.scaled?dw:nullptr,ref,c.k,pc,128,rp);};
  auto blocked=[&](){w4a16_gemv_blocked_partial_kernel<<<grid,256,shared>>>(dx,c.separate?c.k:0,reinterpret_cast<int32_t*>(packed),dz,ds,di,sq,sz,ss,c.scaled?dw:nullptr,out,c.k,pc,128,rp);};
  ...
  printf("K=%d N=%d slots=%d splits=%d original_ms=%g blocked_ms=%g bit_mismatches=%d\n",c.k,c.n,c.slots,c.splits,times[0],times[1],bad);if(bad)return 2;
