...
#include "gemv_constants_probe.cuh"
...
int main(){
 ...
 for(auto c:{Case{2048,5120,1,4,false,false},Case{4096,2048,1,8,false,false},Case{2048,1536,8,2,false,false},Case{768,2048,8,1,true,true},Case{2048,5120,1,3,false,false}}){
  ...
  auto base=w4a16_gemv_pair_kernel<true,false,128,true>;
  decltype(base) constants, resident4;
  if(c.k==2048 && c.n==5120 && c.splits==4) { constants=w4a16_gemv_constants_kernel<2048,640,512>; resident4=w4a16_gemv_constants_kernel<2048,640,512,4>; }
  else if(c.k==4096) { constants=w4a16_gemv_constants_kernel<4096,256,512>; resident4=w4a16_gemv_constants_kernel<4096,256,512,4>; }
  else if(c.k==2048 && c.n==1536) { constants=w4a16_gemv_constants_kernel<2048,192,1024>; resident4=w4a16_gemv_constants_kernel<2048,192,1024,4>; }
  else if(c.k==768) { constants=w4a16_gemv_constants_kernel<768,256,768>; resident4=w4a16_gemv_constants_kernel<768,256,768,4>; }
  else { constants=w4a16_gemv_constants_kernel<2048,640,688>; resident4=w4a16_gemv_constants_kernel<2048,640,688,4>; }
  decltype(base) variants[]={base,w4a16_gemv_constants_kernel<>,constants,resident4};
