...
(latency depends on shape+dtype, not trained values). A checkpoint is benchmarked at
its *native* config (e.g. pi05_libero_base = H50), matching the LIBERO deployment;
shape scanning (``--views/--image-size/--action-horizon/--num-flow-steps/
--max-token-len``) is a synthetic-only knob. Synthetic mode covers **L0/L1/L2**: L2
wraps the engine in synthetic processors (a fixed-length tokenizer + identity
unnormalize, so its actions are latency-only). L3 attaches to a running server
(``--host/--port``) and needs no local weights — serve with ``--random-weights`` for a
fully checkpoint-free L3.
...
# benchmark.md primary/worst-case LIBERO prompts (T = PaliGemma token count).
...
def parse_args() -> argparse.Namespace:
    ...
    # Architecture overrides — random mode only (a checkpoint is fixed to its config).
    ...
    p.add_argument("--action-horizon", type=int, help="action horizon (random)")
    p.add_argument(
        "--action-dim",
        type=int,
...
def main() -> None:
    ...
    # Architecture overrides fix synthetic shapes; a checkpoint is bound to its config.
    override_flags = {
        "--views": args.views,
        "--image-size": args.image_size,
        "--action-horizon": args.action_horizon,
