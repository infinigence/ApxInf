...
python3 benchmarks/qwen38_4090/evaluation/run_evaluation.py \
  ...
  --implementation-revision worktree-iter8 --backend apxinf \
...
- raw JSONL SHA-256: `df2fa1cb907bd1b0162343137f654d1df4d89dc0494e23b19fd4ae90bf77cda1`
...
|---|---:|---:|---:|---:|---:|---:|
| text-perf-1024 | 1,024 | 1.382 s | 740.74 tok/s | 55.31 ms | 18.08 tok/s | 24,012 MiB |
| text-perf-2048 | 2,048 | 2.777 s | 737.36 tok/s | 57.73 ms | 17.32 tok/s | 24,012 MiB |
| text-perf-4096 | 4,096 | 5.611 s | 730.03 tok/s | 62.45 ms | 16.01 tok/s | 24,012 MiB |
| text-perf-8192 | 8,192 | 11.449 s | 715.50 tok/s | 71.88 ms | 13.91 tok/s | 24,012 MiB |
| text-perf-16384 | 16,384 | 23.812 s | 688.06 tok/s | 90.76 ms | 11.02 tok/s | 24,012 MiB |
...
Compared with iteration 7, 64-column tiling reduced base TTFT by approximately 1.6–2.1% and 32K diagnostic TTFT by approximately 1.2%, while preserving exact trajectory correctness.
...
|---:|---:|---:|---:|---:|---:|---:|
| 32,640 | 1 required probe | 1/1 | 128 | 51.156 s | 128.21 ms | 67.439 s |
...
The answer row reports `functional_pass=true`, `prompt_tokens=32640`, `completion_tokens=128`, `ttft_s=51.15634301304817`, `tpot_s=0.12820937936230908`, and `e2e_s=67.4390151873231`.
...
- `target/iterate8-profile.nsys-rep` (137,430 bytes)
- `target/iterate8-profile.sqlite` (Nsight export)
...
/opt/nvidia/nsight-compute/2025.1.1/host/target-linux-x64/nsys profile \
  ...
  --output=target/iterate8-profile \
...
The paired MLP projections remain the largest individual GEMMs. The 64-column recurrence tile is the optimization target: its lower shared-memory footprint raises block residency without changing the recurrence math. Weight traffic, launch/tail overhead, and the serial layer sequence remain material.

### Memory bandwidth and bottlenecks

The evaluator hardware sampler observed `memory_controller_util_max_pct` 76–77% and `memory_used_peak_mib=24012` during the run. Nsight Systems measured the captured startup/short request's host-to-device traffic at 747.553 MB across 78 copies (average 9.584 MB; maximum 44.564 MB), with 224.914 ms aggregate copy time. These copies are dominated by initialization and host transfers, not the steady-state decode loop.
...
- ApxInf prefill: 679–729 tok/s across 1K–16K base cells; 32.64K TTFT corresponds to 631.0 prompt tok/s (`32640 / 51.15634301304817`).
- ApxInf decode: 11.03–18.10 tok/s across base cells; 32.64K context decode is 7.81 tok/s (`1 / 0.12820937936230908`).
...
- `cargo check --workspace --locked` passed for the 64-column recurrence source; the official run preserved 256/256 trajectory correctness.
...
- The official run is tied to the `worktree-iter8` build. A formal run must use the complete commit SHA.
