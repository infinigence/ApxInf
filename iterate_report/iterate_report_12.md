# Iteration Report 12 — ApxInf Qwen3.8-27B on RTX 4090

Date: 2026-08-24  
Implementation revision: `f4793ee6d7782c61a55fb2db95cc52d438b5d473` plus the measured validated iteration-9 fully-unrolled recurrence baseline plus guarded baseline-preserving residual path in `crates/apxinf-model/src/qwen35/cuda.rs` (worktree build).  
Run id: `iterate12-baseline`
...
python3 benchmarks/qwen38_4090/evaluation/run_evaluation.py \
  ...
  --implementation-revision worktree-iter12 --backend apxinf \
...
- raw JSONL SHA-256: `9af0c093fad4bdec516a5e89f1e1bef31c3a2d0fddcf923a3dd6114027f56a06`
...
|---|---:|---:|---:|---:|---:|---:|
| text-perf-1024 | 1,024 | 1.385 s | 739.84 tok/s | 55.26 ms | 18.10 tok/s | 24,012 MiB |
| text-perf-2048 | 2,048 | 2.780 s | 736.56 tok/s | 57.61 ms | 17.36 tok/s | 24,012 MiB |
| text-perf-4096 | 4,096 | 5.612 s | 729.81 tok/s | 62.34 ms | 16.04 tok/s | 24,012 MiB |
| text-perf-8192 | 8,192 | 11.453 s | 715.30 tok/s | 71.78 ms | 13.93 tok/s | 24,012 MiB |
| text-perf-16384 | 16,384 | 23.817 s | 687.92 tok/s | 90.65 ms | 11.03 tok/s | 24,012 MiB |
...
|---:|---:|---:|---:|---:|---:|---:|
| 32,640 | 1 required probe | 1/1 | 128 | 51.172 s | 128.10 ms | 67.441 s |
...
The answer row reports `functional_pass=true`, `prompt_tokens=32640`, `completion_tokens=128`, `ttft_s=51.172467201948166`, `tpot_s=0.12809938931559015`, and `e2e_s=67.44117891043425`.
...
- `target/iterate12-profile was not persisted
- `target/iterate12-profile unavailable` (Nsight export)
...
- ApxInf prefill: 679–729 tok/s across 1K–16K base cells; 32.64K TTFT corresponds to 631.0 prompt tok/s (`32640 / 51.172467201948166`).
- ApxInf decode: 11.03–18.10 tok/s across base cells; 32.64K context decode is 7.81 tok/s (`1 / 0.12809938931559015`).
...
- The official run is tied to the `worktree-iter12` build. A formal run must use the complete commit SHA. The provided vLLM endpoint at `http://0.0.0.0:8000` was reachable, but its vLLM evaluator adapter returned no successful `text-perf-1024` row, so an authoritative 1.2x comparison remains unavailable.
