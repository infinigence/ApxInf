# Iteration Report 4 — ApxInf Qwen3.8-27B on RTX 4090

Date: 2026-08-24  
Implementation revision: `f4793ee6d7782c61a55fb2db95cc52d438b5d473` plus the measured reusable token-upload staging change in `crates/apxinf-model/src/qwen35/cuda.rs` (worktree build).  
Run id: `iterate4-staging`

## Problem and decision

Iteration 3 established 32K capacity. This iteration removes the per-upload `Vec<u8>` allocation in the Qwen CUDA path by retaining a fixed host staging buffer for token IDs. The 32K capacity and 128-row prefill workspace remain unchanged. The paired W4A16 tensor-core decode path remains enabled.
...
python3 benchmarks/qwen38_4090/evaluation/run_evaluation.py \
  ...
  --implementation-revision worktree-iter4 --backend apxinf \
...
- raw JSONL SHA-256: `352a5cab1b26c4e508f14c36c637509b8899f55544bf847fd0c9b6d62033c3e1`
...
|---|---:|---:|---:|---:|---:|---:|
| text-perf-1024 | 1,024 | 1.403 s | 729.70 tok/s | 55.24 ms | 18.10 tok/s | 24,012 MiB |
| text-perf-2048 | 2,048 | 2.818 s | 726.79 tok/s | 57.60 ms | 17.36 tok/s | 24,012 MiB |
| text-perf-4096 | 4,096 | 5.685 s | 720.44 tok/s | 62.32 ms | 16.05 tok/s | 24,012 MiB |
| text-perf-8192 | 8,192 | 11.599 s | 706.26 tok/s | 71.76 ms | 13.93 tok/s | 24,012 MiB |
| text-perf-16384 | 16,384 | 24.115 s | 679.40 tok/s | 90.63 ms | 11.03 tok/s | 24,012 MiB |
...
Compared with iteration 3, reusable staging improved measured TTFT by 0.1–0.2% and TPOT by 0.1–0.2% in this single-repeat run; the change is intentionally small because allocation overhead is a minor component beside recurrence and weight traffic.
...
|---:|---:|---:|---:|---:|---:|---:|
| 32,640 | 1 required probe | 1/1 | 128 | 51.750 s | 128.07 ms | 68.016 s |
...
The answer row reports `functional_pass=true`, `prompt_tokens=32640`, `completion_tokens=128`, `ttft_s=51.75025140494108`, `tpot_s=0.1280749718269964`, and `e2e_s=68.01585571467876`.
...
- `target/iterate4-profile.nsys-rep` (210,234 bytes)
- `target/iterate4-profile.sqlite` (Nsight export)
...
- ApxInf prefill: 679–729 tok/s across 1K–16K base cells; 32.64K TTFT corresponds to 631.0 prompt tok/s (`32640 / 51.75025140494108`).
- ApxInf decode: 11.03–18.10 tok/s across base cells; 32.64K context decode is 7.81 tok/s (`1 / 0.1280749718269964`).
...
- The official run is tied to the pre-final worktree build's `worktree-iter4` identity. The final rebuilt binary is source-equivalent for the capacity configuration; rerun the official command after creating a commit for a formal revision identity.
