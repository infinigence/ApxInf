# Iteration Report 5 — ApxInf Qwen3.8-27B on RTX 4090

Date: 2026-08-24  
Implementation revision: `f4793ee6d7782c61a55fb2db95cc52d438b5d473` plus the measured delta recurrence barrier elision change in `crates/apxinf-model/src/qwen35/cuda.rs` (worktree build).  
Run id: `iterate5-sync`

## Problem and decision

Iteration 3 established 32K capacity. This iteration removes the unnecessary final `__syncthreads()` from the delta recurrence kernel. For `seq=1` decode, no subsequent iteration reads the shared q/k vectors, so the barrier is conditional on `s + 1 < seq`; state updates and output accumulation are unchanged. The 32K capacity, 128-row prefill workspace, and reusable token staging remain unchanged.
...
python3 benchmarks/qwen38_4090/evaluation/run_evaluation.py \
  ...
  --implementation-revision worktree-iter5 --backend apxinf \
...
- `benchmarks/qwen38_4090/evaluation/runs/iterate4-staging/submission.json`
- `benchmarks/qwen38_4090/evaluation/runs/iterate4-staging/raw.jsonl`
- `benchmarks/qwen38_4090/evaluation/runs/iterate4-staging/environment.json`
...
- raw JSONL SHA-256: `11e9d2e3e1e2e0d966c6bbbd91bdac545e590d07067e19d9777ef2c29f7d8b26`
...
|---|---:|---:|---:|---:|---:|---:|
| text-perf-1024 | 1,024 | 1.403 s | 729.68 tok/s | 55.24 ms | 18.10 tok/s | 24,012 MiB |
| text-perf-2048 | 2,048 | 2.818 s | 726.71 tok/s | 57.62 ms | 17.36 tok/s | 24,012 MiB |
| text-perf-4096 | 4,096 | 5.687 s | 720.30 tok/s | 62.34 ms | 16.04 tok/s | 24,012 MiB |
| text-perf-8192 | 8,192 | 11.602 s | 706.09 tok/s | 71.78 ms | 13.93 tok/s | 24,012 MiB |
| text-perf-16384 | 16,384 | 24.125 s | 679.14 tok/s | 90.65 ms | 11.03 tok/s | 24,012 MiB |
...
Compared with iteration 4, the final recurrence barrier was removed; measured rates stayed within run-to-run noise measured TTFT by 0.1–0.2% and TPOT by 0.1–0.2% in this single-repeat run; the change is intentionally small because allocation overhead is a minor component beside recurrence and weight traffic.
...
|---:|---:|---:|---:|---:|---:|---:|
| 32,640 | 1 required probe | 1/1 | 128 | 51.771 s | 128.10 ms | 68.040 s |
...
The answer row reports `functional_pass=true`, `prompt_tokens=32640`, `completion_tokens=128`, `ttft_s=51.77096513658762`, `tpot_s=0.12810262016893373`, and `e2e_s=68.04007469117641`.
...
- `target/iterate5-profile.nsys-rep` (210,234 bytes)
- `target/iterate5-profile.sqlite` (Nsight export)
...
/opt/nvidia/nsight-compute/2025.1.1/host/target-linux-x64/nsys profile \
  ...
  --output=target/iterate4-profile \
...
- ApxInf prefill: 679–729 tok/s across 1K–16K base cells; 32.64K TTFT corresponds to 631.0 prompt tok/s (`32640 / 51.77096513658762`).
- ApxInf decode: 11.03–18.10 tok/s across base cells; 32.64K context decode is 7.81 tok/s (`1 / 0.12810262016893373`).
...
- `python3 benchmarks/qwen38_4090/evaluation/test.py check` passed after the optimization; its Rust check and assignment package checks are green.
...
- The official run is tied to the pre-final worktree build's `worktree-iter5` identity. The final rebuilt binary is source-equivalent for the capacity configuration; rerun the official command after creating a commit for a formal revision identity.
...
python3 benchmarks/qwen38_4090/evaluation/run_evaluation.py \
  ...
  --context-dataset benchmarks/qwen38_4090/evaluation/.cache/context-iter3 \
  --model-dir ../model/qwen --base-url http://127.0.0.1:8002 \
  --implementation-name apxinf-student --implementation-revision <full-commit-sha> \
  --backend apxinf --profile public_calibration \
  --trajectory-reference target/iterate2-trajectory-regression.json \
  --run-context --warmups 0 --repeats 1 --timeout 1800 \
  --run-id iterate4-staging --output-dir benchmarks/qwen38_4090/evaluation/runs
```
