# Iteration Report 6 — ApxInf Qwen3.8-27B on RTX 4090

Date: 2026-08-24  
Implementation revision: `f4793ee6d7782c61a55fb2db95cc52d438b5d473` plus the measured combined token-position upload experiment in `crates/apxinf-model/src/qwen35/cuda.rs` (worktree build).  
Run id: `iterate6-control`

## Problem and decision

Iteration 3 established 32K capacity. This iteration experimentally combines token IDs and the current position into one device allocation and one host-to-device copy. The change was rolled back after the official run exposed a correctness regression: the public trajectory fell from 256/256 to 164/256 even though functional cases still passed. The committed source remains the iteration-5 known-good path; this report preserves the negative result and rollback decision.
...
python3 benchmarks/qwen38_4090/evaluation/run_evaluation.py \
  ...
  --implementation-revision worktree-iter6 --backend apxinf \
...
- raw JSONL SHA-256: `5cafa3a9ef84e65dfc1020355c61c6f9b99ca1062a09f9c4d93148acd436ab95`
...
| Public trajectory | 164/256 (negative-control run; source rolled back) |
...
|---|---:|---:|---:|---:|---:|---:|
| text-perf-1024 | 1,024 | 1.402 s | 730.65 tok/s | 55.28 ms | 18.09 tok/s | 24,012 MiB |
| text-perf-2048 | 2,048 | 2.814 s | 727.69 tok/s | 57.64 ms | 17.35 tok/s | 24,012 MiB |
| text-perf-4096 | 4,096 | 5.684 s | 720.63 tok/s | 62.42 ms | 16.02 tok/s | 24,012 MiB |
| text-perf-8192 | 8,192 | 11.603 s | 706.01 tok/s | 71.86 ms | 13.92 tok/s | 24,012 MiB |
| text-perf-16384 | 16,384 | 24.123 s | 679.18 tok/s | 90.73 ms | 11.02 tok/s | 24,012 MiB |
...
Compared with iteration 5, the combined upload experiment produced similar raw latency but changed the token trajectory. It is rejected and not retained in the final source.
...
|---:|---:|---:|---:|---:|---:|---:|
| 32,640 | 1 required probe | 1/1 | 128 | 51.774 s | 128.13 ms | 68.047 s |
...
```text
KEY-EARLY-767211 context\n<think>\nThe user is asking me to find the value of TARGET_KEY from the document. Let me look at the document carefully.\n\nIn the text, there is a section marked with 【课程私有档案】that states:\n\n"【课程私有档案】唯一目标字段 TARGET_KEY 的值为 KEY-EARLY-767211。"\n\nThe user's question asks: "TARGET_KEY 的值是什么？只输出完整字段值。"\n\nAnd the output must start with "KEY-EARLY-767
```

The answer row reports `functional_pass=true`, `prompt_tokens=32640`, `completion_tokens=128`, `ttft_s=51.77370202541351`, `tpot_s=0.12813174143785566`, and `e2e_s=68.04651339352131`.
...
- `target/iterate6-profile.nsys-rep` (146,201 bytes)
- `target/iterate6-profile.sqlite` (Nsight export)
...
/opt/nvidia/nsight-compute/2025.1.1/host/target-linux-x64/nsys profile \
  ...
  --output=target/iterate6-profile \
...
- ApxInf prefill: 679–729 tok/s across 1K–16K base cells; 32.64K TTFT corresponds to 631.0 prompt tok/s (`32640 / 51.77370202541351`).
- ApxInf decode: 11.03–18.10 tok/s across base cells; 32.64K context decode is 7.81 tok/s (`1 / 0.12813174143785566`).
...
- `python3 benchmarks/qwen38_4090/evaluation/test.py check` passed for the restored known-good source. The combined-upload experiment was rejected by the 164/256 trajectory result.
...
- The official run is tied to the pre-final worktree build's `worktree-iter6` identity. The final rebuilt binary is source-equivalent for the capacity configuration; rerun the official command after creating a commit for a formal revision identity.
