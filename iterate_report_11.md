# Iteration Report 11 — ApxInf Qwen3.8-27B on RTX 4090

Date: 2026-08-24  
Implementation revision: `f4793ee6d7782c61a55fb2db95cc52d438b5d473` plus the measured validated iteration-9 fully-unrolled recurrence baseline in `crates/apxinf-model/src/qwen35/cuda.rs` (worktree build).  
Run id: `iterate11-validated`
...
python3 benchmarks/qwen38_4090/evaluation/run_evaluation.py \
  ...
  --implementation-revision worktree-iter11 --backend apxinf \
...
- raw JSONL SHA-256: `e1065877b0e932f66945a71b683555cda34f852f4d71b79ad675ad3038903b6d`
...
|---|---:|---:|---:|---:|---:|---:|
| text-perf-1024 | 1,024 | 1.384 s | 740.01 tok/s | 55.30 ms | 18.08 tok/s | 24,012 MiB |
| text-perf-2048 | 2,048 | 2.779 s | 736.93 tok/s | 57.63 ms | 17.35 tok/s | 24,012 MiB |
| text-perf-4096 | 4,096 | 5.608 s | 730.35 tok/s | 62.36 ms | 16.04 tok/s | 24,012 MiB |
| text-perf-8192 | 8,192 | 11.445 s | 715.79 tok/s | 71.78 ms | 13.93 tok/s | 24,012 MiB |
| text-perf-16384 | 16,384 | 23.804 s | 688.29 tok/s | 90.65 ms | 11.03 tok/s | 24,012 MiB |
...
|---:|---:|---:|---:|---:|---:|---:|
| 32,640 | 1 required probe | 1/1 | 128 | 51.148 s | 128.10 ms | 67.417 s |
...
The answer row reports `functional_pass=true`, `prompt_tokens=32640`, `completion_tokens=128`, `ttft_s=51.147872902452946`, `tpot_s=0.1281011790975811`, and `e2e_s=67.41678974032402`.
...
- `target/iterate11-profile.nsys-rep` (229,820 bytes)
- `target/iterate11-profile.sqlite` (Nsight export)
...
/opt/nvidia/nsight-compute/2025.1.1/host/target-linux-x64/nsys profile \
  ...
  --output=target/iterate11-profile \
...
The paired MLP projections remain the largest individual GEMMs. The fully-unrolled recurrence is still dominated by weight traffic, launch/tail overhead, and the serial layer sequence. Nsight kernel rows were unavailable for this startup-only capture; prior runtime event measurements remain the kernel-level reference.

### Memory bandwidth and bottlenecks

The evaluator hardware sampler observed `memory_controller_util_max_pct` 76–77% and `memory_used_peak_mib=24012` during the run. Nsight Systems measured the captured startup/short request's host-to-device traffic at Kernel rows were unavailable in the startup-only export because the profiler terminated during post-capture processing; the persisted `target/iterate9-profile.nsys-rep` is retained. CUDA API analysis recorded `cudaMemcpy` at 1,051.636 ms (93.0%), `cudaMalloc` at 76.953 ms (6.8%), and `cudaFree` at 2.431 ms (0.2%). Host-to-device traffic totalled 10,307.006 MB across 714 copies, with 996.621 ms aggregate copy time. These copies are dominated by initialization and host transfers, not the steady-state decode loop.
...
The contract's authoritative performance reference is the best valid same-round vLLM median per cell. A provided vLLM endpoint at `http://0.0.0.0:8000` was reachable and advertised model `../qwen`, but the evaluator adapter returned no successful performance row; a local vLLM startup attempt also failed KV-cache initialization with CUDA OOM. Therefore:

- ApxInf prefill: 679–729 tok/s across 1K–16K base cells; 32.64K TTFT corresponds to 631.0 prompt tok/s (`32640 / 51.147872902452946`).
- ApxInf decode: 11.03–18.10 tok/s across base cells; 32.64K context decode is 7.81 tok/s (`1 / 0.1281011790975811`).
- The required `>=1.2×` comparison is **unverified**, not passed: no valid vLLM performance rows were available. A baseline row is required before claiming the threshold.

## Negative controls and known limitations

- `cargo check --workspace --locked` passed for the fully-unrolled source; the official run preserved 256/256 trajectory correctness.
...
- The official run is tied to the `worktree-iter11` build. A formal run must use the complete commit SHA. The provided vLLM endpoint at `http://0.0.0.0:8000` was reachable, but its vLLM evaluator adapter returned no successful `text-perf-1024` row, so an authoritative 1.2x comparison remains unavailable.
