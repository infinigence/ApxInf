 ...
 private:
  ...
  ComputeFragment per_token_channel_scale_accumulator_(
    ...
    for (int i = 0; i < ComputeFragment::kElements; ++i) {
      // Preserve the standalone dequantization path's IEEE operation order.
      // A one-ULP BF16 difference here compounds through the recurrent action
      // denoiser, so association is part of the W8A8 correctness contract.
      result[i] = (accum[i] * scale_row) * scale_col[i];
    }
