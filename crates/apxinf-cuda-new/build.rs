...
fn write_arch_header(out: &Path, selection: &ArchSelection) -> PathBuf {
    ...
    let mut header = String::from(
        "#pragma once\n#include <cstddef>\n#include <cstdint>\nnamespace apxinf::gemm {\n\
         ...
         constexpr uint64_t kDeviceFeatureCutlassSm80W8a8 = UINT64_C(1) << 3;\n\
...
fn main() {
        ...
        .any(|target| is_cutlass_sm100_family(&target.cutlass_arch))
    {
        ...
        cutlass_sm100_sources.push(adapters.join("attention/providers/cutlass.cu"));
    }
    ...
    for (index, source) in generic_sources
        ...
        .chain(
            fa2_sources
                .into_iter()
                .map(|source| (source, NativeSourceKind::Fa2)),
        )
        .chain(
            fa2_e4m3_sources
                .into_iter()
                .map(|source| (source, NativeSourceKind::Fa2E4m3)),
        )
        .enumerate()
    {
        let (source, source_kind) = source;
        let is_cutlass = matches!(
            source_kind,
            NativeSourceKind::CutlassSm80 | NativeSourceKind::CutlassSm100
        );
        let is_fa2 = matches!(source_kind, NativeSourceKind::Fa2);
        let is_fa2_e4m3 = matches!(source_kind, NativeSourceKind::Fa2E4m3);
        ...
        command.args(match source_kind {
            ...
            NativeSourceKind::CutlassSm100 | NativeSourceKind::Fa2E4m3 => &cutlass_sm100_codegen,
            NativeSourceKind::Fa2 => &fa2_codegen,
        });
        ...
        if !cutlass_sm100_codegen.is_empty() {
