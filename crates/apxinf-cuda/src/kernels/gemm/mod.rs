...
pub fn matmul(ctx: &CudaContext, activation: &Tensor, weight: &Tensor) -> Result<Tensor> {
    ...
    // implement_port r6 (analysis/r6_change_selection): plain alloc — the
    // cuBLAS GEMM below runs with beta = 0.0 and fully overwrites the output,
    // so the synchronous alloc_zeros memset was dead work.
