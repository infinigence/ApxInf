...
extern "C" int apxinf_static_fa2_f16_causal_hdim128(
    const void* q, const void* k, const void* v, void* output,
    void* softmax_lse, int batch, int query_tokens, int key_tokens,
    int query_heads, int kv_heads, float softmax_scale, cudaStream_t stream) {
  return apxinf::cuda::cutlass_ops::fa2_f16_causal_hdim128(
      q, k, v, output, softmax_lse, batch, query_tokens, key_tokens,
      query_heads, kv_heads, softmax_scale, stream);
}

extern "C" int apxinf_static_fa2_f16_causal_cached_hdim128(
    const void* q, const void* k, const void* v, void* output,
    void* softmax_lse, int batch, int query_tokens, int key_tokens, const void* used_k,
    int query_heads, int kv_heads, float softmax_scale, cudaStream_t stream) {
  return apxinf::cuda::cutlass_ops::fa2_f16_causal_cached_hdim128(
      q, k, v, output, softmax_lse, batch, query_tokens, key_tokens, static_cast<const int*>(used_k),
      query_heads, kv_heads, softmax_scale, stream);
}
