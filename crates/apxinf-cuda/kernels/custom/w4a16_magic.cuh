#pragma once
// Exact unsigned nibble conversion: (0x4b000000 | q) - 2^23 yields q.
// Sixteen loads are issued before consumption; FMA and reduction order stay fixed.
__device__ __forceinline__ float awq_magic_float(uint32_t v) {
  uint32_t bits;
  asm("lop3.b32 %0, %1, 0x0000000f, 0x4b000000, 0xea;" : "=r"(bits) : "r"(v));
  return __uint_as_float(bits)-8388608.f;
}
template<bool blocked=false, bool pdl=false>
__global__ void __launch_bounds__(256) w4a16_gemv_magic_kernel(
