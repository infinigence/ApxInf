...
__global__ void Marlin(
    ...
    bool use_fp32_reduce,  // whether to use fp32 global reduce
    int max_shared_mem) {
  if constexpr (fixed_awq) {
    // Exact adapter configuration: this branch removes runtime alternatives
    // without changing the tile schedule or FP32 reduction order.
    static_assert(std::is_same<scalar_t, half>::value &&
                  w_type_id == vllm::kU4.id() &&
                  s_type_id == vllm::kFloat16.id() && group_blocks == 8 &&
                  !is_zp_float && !fuse_silu);
  ...
  if constexpr (w_type == vllm::kFE2M1f) {
    ...
    static_assert(s_type == vllm::kBFloat16 ||
                  (s_type == vllm::kFloat16 && w_type == vllm::kU4 && group_blocks == 8 && !is_zp_float));
  ...
  const int group_size = fixed_awq ? 128 :
  ...
  #pragma unroll
    for (int j = 0; j < 4; j++) {
      ...
      } else if constexpr (std::is_same<scalar_t, nv_bfloat16>::value && s_type == vllm::kFloat16) {
        // Preserve checkpoint FP16 scales: integer differences are exact BF16,
        // multiply in FP32 then round once to BF16, matching AutoAWQ dequant.
        const half* scales_half = reinterpret_cast<const half*>(&frag_s[k2][j]);
        const float s0 = __half2float(scales_half[0]);
        const float s1 = __half2float(scales_half[1]);
#pragma unroll
        for (int f = 0; f < 2; ++f) {
          frag_b0[f] = __floats2bfloat162_rn(__bfloat162float(frag_b0[f].x)*s0, __bfloat162float(frag_b0[f].y)*s0);
          frag_b1[f] = __floats2bfloat162_rn(__bfloat162float(frag_b1[f].x)*s1, __bfloat162float(frag_b1[f].y)*s1);
        }
      } else if constexpr (group_blocks != -1) {
    ...
    for (int i = 0;
      ...
      if (row < block_num_valid_tokens) {
        ...
        if constexpr(fuse_silu) {
          static_assert(std::is_same<scalar_t,half>::value && thread_n_blocks%8==0);
          int column=c_gl_wr%c_gl_stride;
          if(column%16<8) {
            int4 gate=sh_red[c_sh_rd],up=sh_red[c_sh_rd+8],result;
            const half* gp=reinterpret_cast<const half*>(&gate);
            const half* uptr=reinterpret_cast<const half*>(&up);
            half* dst=reinterpret_cast<half*>(&result);
#pragma unroll
            for(int lane=0;lane<8;++lane) {
              float g=__bfloat162float(__float2bfloat16(__half2float(gp[lane])));
              float u=__bfloat162float(__float2bfloat16(__half2float(uptr[lane])));
              dst[lane]=__float2half(__bfloat162float(__float2bfloat16((g/(1.f+expf(-g)))*u)));
            }
            C[sorted_row*(c_gl_stride/2)+(column/16)*8+column%8]=result;
          }
        } else if (use_atomic_add && slice_count > 1 || mul_topk_weights) {
