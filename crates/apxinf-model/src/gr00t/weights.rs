...
use super::action_weights::Gr00tActionHeadWeights;
...
fn validate_backbone_config(config: &Gr00tConfig, backbone: &Qwen3VLConfig) -> Result<()> {
    ...
    if !backbone.text.mrope_interleaved {
        return Err(Error::Other(
            "GR00T N1.7 requires the interleaved Qwen3-VL mRoPE layout".into(),
        ));
    }
    Ok(())
}
...
fn validate_merger(
    name: &str,
    merger: &super::backbone::vision_weights::Qwen3VLMerger,
    norm_width: usize,
    hidden_width: usize,
    output_width: usize,
) -> Result<()> {
