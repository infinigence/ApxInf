...
impl Qwen3VLVisionWeights {
    ...
    /// Consume the vision tensors from a shared Qwen3-VL checkpoint map.
    pub(crate) fn take_from_map(
        ...
        for i in 0..depth {
            ...
            blocks.push(Qwen3VLVisionBlock {
                norm1_w: take(&format!("{p}.norm1.weight"), tensors)?,
                norm1_b: take(&format!("{p}.norm1.bias"), tensors)?,
                qkv_w: reshape_linear_weight(&take(&format!("{p}.attn.qkv.weight"), tensors)?)?,
                qkv_b: take(&format!("{p}.attn.qkv.bias"), tensors)?,
                proj_w: reshape_linear_weight(&take(&format!("{p}.attn.proj.weight"), tensors)?)?,
                proj_b: take(&format!("{p}.attn.proj.bias"), tensors)?,
                norm2_w: take(&format!("{p}.norm2.weight"), tensors)?,
                norm2_b: take(&format!("{p}.norm2.bias"), tensors)?,
                fc1_w: reshape_linear_weight(&take(
                    &format!("{p}.mlp.linear_fc1.weight"),
                    tensors,
                )?)?,
                fc1_b: take(&format!("{p}.mlp.linear_fc1.bias"), tensors)?,
                fc2_w: reshape_linear_weight(&take(
                    &format!("{p}.mlp.linear_fc2.weight"),
                    tensors,
                )?)?,
                fc2_b: take(&format!("{p}.mlp.linear_fc2.bias"), tensors)?,
            });
        }
        ...
        let merger = load_merger("model.visual.merger", tensors, false)?;
...
fn reshape_5d_to_2d(t: &Tensor, rows: usize, cols: usize) -> Result<Tensor> {
    ...
    let expected = dims
        ...
        .ok_or_else(|| Error::Other("Qwen3-VL vision weight size overflow".into()))?;
    let requested = rows
        .checked_mul(cols)
        .ok_or_else(|| Error::Other("Qwen3-VL reshaped weight size overflow".into()))?;
