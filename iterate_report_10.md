# Iteration Report 10 — ApxInf Qwen3.8-27B on RTX 4090

Date: 2026-08-24  
Implementation revision: `f4793ee6d7782c61a55fb2db95cc52d438b5d473` plus the measured iteration-9 fully-unrolled recurrence baseline in `crates/apxinf-model/src/qwen35/cuda.rs` (worktree build).  
Run id: `iterate10-baseline`
...
python3 benchmarks/qwen38_4090/evaluation/run_evaluation.py \
  ...
  --implementation-revision worktree-iter10 --backend apxinf \
...
- raw JSONL SHA-256: `746dfba8094641cdc799967efd8a97f2dc53280cd00bc056b77db8af9eff1a6c`
...
|---|---:|---:|---:|---:|---:|---:|
| text-perf-1024 | 1,024 | 1.382 s | 740.72 tok/s | 55.25 ms | 18.10 tok/s | 24,012 MiB |
| text-perf-2048 | 2,048 | 2.776 s | 737.72 tok/s | 57.59 ms | 17.36 tok/s | 24,012 MiB |
| text-perf-4096 | 4,096 | 5.609 s | 730.23 tok/s | 62.41 ms | 16.02 tok/s | 24,012 MiB |
| text-perf-8192 | 8,192 | 11.449 s | 715.52 tok/s | 71.81 ms | 13.93 tok/s | 24,012 MiB |
| text-perf-16384 | 16,384 | 23.816 s | 687.93 tok/s | 90.67 ms | 11.03 tok/s | 24,012 MiB |
...
|---:|---:|---:|---:|---:|---:|---:|
| 32,640 | 1 required probe | 1/1 | 128 | 51.164 s | 128.11 ms | 67.434 s |
...
The answer row reports `functional_pass=true`, `prompt_tokens=32640`, `completion_tokens=128`, `ttft_s=51.1640010997653`, `tpot_s=0.1281067064310622`, and `e2e_s=67.4336289986968`.
...
- `target/iterate10-profile.nsys-rep` (165,452 bytes)
- `target/iterate10-profile.sqlite` (Nsight export)
...
/opt/nvidia/nsight-compute/2025.1.1/host/target-linux-x64/nsys profile \
  ...
  --output=target/iterate10-profile \
...
- ApxInf prefill: 679–729 tok/s across 1K–16K base cells; 32.64K TTFT corresponds to 631.0 prompt tok/s (`32640 / 51.1640010997653`).
- ApxInf decode: 11.03–18.10 tok/s across base cells; 32.64K context decode is 7.81 tok/s (`1 / 0.1281067064310622`).
...
- The official run is tied to the `worktree-iter10` build. A formal run must use the complete commit SHA. The provided vLLM endpoint at `http://0.0.0.0:8000` was reachable, but its vLLM evaluator adapter returned no successful `text-perf-1024` row, so an authoritative 1.2x comparison remains unavailable.
